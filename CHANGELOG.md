...
- Fixed CLI crash on Python 3.14 caused by deferred annotation evaluation
  (PEP 649). The `list[str]` type hint in the `secrets set` command failed
  when evaluated through the `synchronicity` wrapper, breaking all CLI
  commands on startup.
...
- Credentials file with overly permissive permissions is now automatically
  repaired to 600 instead of issuing a suppressible warning. If repair fails,
  the CLI refuses to load the token.
